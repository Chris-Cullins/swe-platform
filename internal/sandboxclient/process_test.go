...
func TestDialProcessValidatesCurrentEnvironmentPodAndCredentialIncarnation(t *testing.T) {
	...
	process, closeConnection, err := DialProcess(context.Background(), newClient(env.DeepCopy(), pod.DeepCopy(), secret.DeepCopy()), env.Namespace, env.Name, env.UID)
	...
	if _, _, err := DialProcess(context.Background(), newClient(env.DeepCopy(), wrongOwner, secret.DeepCopy()), env.Namespace, env.Name, env.UID); err == nil || !strings.Contains(err.Error(), "current environment pod") {
	...
	if _, _, err := DialProcess(context.Background(), newClient(env.DeepCopy(), pod.DeepCopy(), staleCredential), env.Namespace, env.Name, env.UID); err == nil || !strings.Contains(err.Error(), "current environment pod") {
		t.Fatalf("stale credential error = %v", err)
	}
	replacementSecret := secret.DeepCopy()
	replacementSecret.UID = "replacement-secret-uid"
	if _, _, err := DialProcess(context.Background(), newClient(env.DeepCopy(), pod.DeepCopy(), replacementSecret), env.Namespace, env.Name, env.UID); err == nil || !strings.Contains(err.Error(), "current environment pod") {
		t.Fatalf("replacement Secret error = %v", err)
	}
	wrongSecretOwner := secret.DeepCopy()
	wrongSecretOwner.OwnerReferences[0].Kind = "Run"
	if _, _, err := DialProcess(context.Background(), newClient(env.DeepCopy(), pod.DeepCopy(), wrongSecretOwner), env.Namespace, env.Name, env.UID); err == nil || !strings.Contains(err.Error(), "current environment pod") {
		t.Fatalf("wrong Secret owner kind error = %v", err)
	}

	if _, _, err := DialProcess(context.Background(), newClient(env.DeepCopy()), env.Namespace, env.Name, types.UID("replaced-environment")); err == nil || !strings.Contains(err.Error(), "current reachable incarnation") {
