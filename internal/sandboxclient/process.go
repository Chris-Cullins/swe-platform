...
func DialProcess(ctx context.Context, reader client.Reader, namespace, environment string, environmentUID types.UID) (sandboxdv1.ProcessServiceClient, func() error, error) {
	...
	if env.UID != environmentUID || env.Status.PodName == "" || env.Status.Endpoints.Sandboxd == "" {
	...
	if pod.UID == "" || !exactEnvironmentOwner(&pod, &env) || pod.Status.PodIP == "" || env.Status.Endpoints.Sandboxd != wantEndpoint {
		return nil, nil, fmt.Errorf("sandboxd endpoint does not identify the current environment pod")
	}
	var secret corev1.Secret
	if err := reader.Get(ctx, types.NamespacedName{Namespace: namespace, Name: "env-" + env.Name + "-sandboxd"}, &secret); err != nil {
		return nil, nil, err
	}
