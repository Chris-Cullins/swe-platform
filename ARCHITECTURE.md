...
| `Environment` | Template/Project selection, explicit hold policy, bounded wake/suspend/activity intents, and up to 32 service declarations, each with an immutable-per-incarnation random `instanceID`. Status owns lifecycle/execution, claim, observation, activity and recovery state; the gateway owns bounded `portalRoutes` and monotonic `nextPortalRouteGeneration`. Inactive routes are denial tombstones preserved by other status writers. |
